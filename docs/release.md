...
- a generic `RightClick AI` selected-text service, an `Open Paper & Notes` Finder PDF service, plus direct built-in Services for `Add to Calendar`, `Draft Response`, `Explain`, `Extract Action Items`, `Polish Draft`, `Rewrite Friendly`, and `Summarize`
