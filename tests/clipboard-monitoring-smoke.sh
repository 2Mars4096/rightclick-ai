...
build_dir="$(mktemp -d "${TMPDIR:-/tmp}/right-click-clipboard-monitoring.XXXXXX")"
trap 'rm -rf "${build_dir}"' EXIT

/usr/bin/xcrun swiftc \
  -module-cache-path "${build_dir}/ModuleCache" \
  -o "${build_dir}/clipboard-monitoring-smoke" \
  "${repo_root}/tests/clipboard-monitoring-smoke.swift" \
...
"${build_dir}/clipboard-monitoring-smoke"
